...
class User < Principal
  ...
  attr_accessible :login,:firstname,:lastname,:mail,:status,:mail_notification
  ...
  scope :user_in_group, lambda {|group|
    group_id = group.is_a?(Group) ? group.id : group.to_i
    where("#{User.table_name}.id IN (SELECT gu.user_id FROM #{table_name_prefix}groups_users#{table_name_suffix} gu WHERE gu.group_id = ? AND gu.user_id = ?)", group_id, User.current.id)
  }
