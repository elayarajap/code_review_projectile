...
class Issue < ActiveRecord::Base
  ...
  validates_presence_of :subject, :priority, :project, :tracker, :author, :status, :estimated_hours

  validates_length_of :subject, :maximum => 255
  validates_inclusion_of :done_ratio, :in => 0..100
  validates :estimated_hours, :numericality => {:greater_than_or_equal_to => 0, :allow_nil => true, :message => "Invalid Estimated Time Format"}
  ...
  def self.update_billable(issue_id,billable_entries)
  ...
  def assignable_versions_updated
    return @assignable_versions if @assignable_versions
