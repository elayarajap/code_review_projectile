...
class Project < ActiveRecord::Base
  ...
  has_many :meetings
  has_many :consolidated_reports

  has_many :sub_projects, :class_name => 'Project',
                   :foreign_key => "parent_id",
                   :inverse_of => :account
  belongs_to :account, :class_name => 'Project',
                          :foreign_key => "parent_id",
                          :inverse_of => :sub_projects
  ...
  validates_presence_of :name, :identifier, :group_mail_address, :estimated_hours, :project_type, :project_method
  validates_format_of :group_mail_address, :with => /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\Z/i , :message => "is not valid",:if => Proc.new { |at| !at.group_mail_address.blank? }
  ...
  validates :estimated_hours, :numericality => true, :allow_nil => true
  ...
  scope :closed, lambda { where(:status => STATUS_CLOSED) }
