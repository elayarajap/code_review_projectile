...
class Attachment < ActiveRecord::Base
  ...
  belongs_to :directory, :class_name => "directory", :foreign_key => "directory_id"
  ...
  def file_with_revision=(incoming_file)
    unless incoming_file.nil?
      @temp_file = incoming_file
      if @temp_file.size > 0
        if @temp_file.respond_to?(:original_filename)
  ...
  def self.attach_files_with_directory(obj, obj1, attachments)
    result = obj.save_attachments_with_directory(attachments, obj1, User.current)
  ...
  def self.attach_files_revision(obj, obj1, attachments)
    result = obj.save_attachments_with_revision(attachments, obj1, User.current)
    obj.attach_saved_attachments
    result
  end
