...
class Query < ActiveRecord::Base
  ...
  def build_from_params(params)
    ...
    if params[:f] && params[:f].include?("user_id") && !params.has_key?(:v)
      params[:v] = {"user_id"=>["me"]}
    end
    ...
    hashcount = hash.count
      ...
      # Users operator * will make their group memebers alone to get enabled [for managers]
      ...
      if @user_in_group_chk!=0
        for groups_available_val in @groups_available
          usersingrp = User.in_group(groups_available_val.id).collect(&:id)
          for usersingrp_val in usersingrp
            users_values << "#{usersingrp_val}"
          end
        end
      end

      if params.include?(:op) && params[:op].include?("user_id") && params[:op]["user_id"]=="*" && @user_in_group_chk!=0
        params[:op]["user_id"] = "="
        if params.include?(:v)
          params[:v]["user_id"]=users_values
        else
          hash_user = {"user_id" => users_values }
          params[:v] = hash_user
        end
      end
      # Users operator * will make their group memebers alone to get enabled [for managers] ends here

  # appending users filter as default for managers and admin
    #@group_manager_chk_query = Group.where(:lastname => "Managers").first
    #@user_in_group_chk_query = User.user_in_group(@group_manager_chk_query.id).size
  #  New condition for checking manager
    user_in_group_nw =  User.current.manager ? 1 : 0
  if !params.include?(:project_id) && !params.include?(:id) && !params.include?(:op)
    if user_in_group_nw!=0 || User.current.admin?
      if hashcount <= 4
          hash["f"] = ["spent_on", "user_id", ""]
          hash["op"] = {"spent_on"=>"*", "user_id"=>"="}
          hash["v"] = {"user_id"=>["me"]}
          hash["action"] = "index"
          hash["controller"] = "timelog"
      end
    end
  end
  # appending users filter as default for managers ends here

    if params[:fields] || params[:f]
      usrgrpfinal = params[:v]
      usrgrpfinal_op = params[:op]
      usrgrpfinal_f = params[:f]
      if params.include?("group_id") && !params["group_id"].blank?
        if params[:op].include?("user_id") && params[:op]["user_id"]!="*"
          # group module in the filter and integration  
          usergrpval = params["group_id"]
          @user_in_group_filter_val = User.in_group(usergrpval)
          userids = []
          for user_in_group_filter_vals in @user_in_group_filter_val
            userids << "#{user_in_group_filter_vals.id}"
          end
          usrgrpfinal["user_id"] = userids
          # group module in the filter and integration ends here
        end
      end
      self.filters = {}
      add_filters(params[:fields] || params[:f], params[:operators] || params[:op], usrgrpfinal)
    else
  ...
  def available_filters
    unless @available_filters
      ...
      @available_filters.each do |field, options|
        #@manager_entry = Group.where(:lastname => "Managers").first
        #@current_user_check = User.user_in_group(@manager_entry.id).size

        user_in_group_nw =  User.current.manager ? 1 : 0

        if(field == "user_id" && project_id == nil)
          if (user_in_group_nw==0 && !User.current.admin?)
            @available_filters.delete(field)
          end
        end
        ...
        if((field == "author_id") || (field == "cf_6") || (field == "status_id") || (field == "tracker_id") || (field == "priority_id") || (field == "assigned_to_id") || (field == "fixed_version_id") || (field == "spent_on") || (field == "user_id") || (field == "activity_id") || (field == "project_id") || (field == "subject"))
  ...
  def available_filters_temp
    unless @available_filters
      initialize_available_filters
      @available_filters.each do |field, options|
        if((field == "status_id") || (field == "tracker_id") || (field == "priority_id") || (field == "assigned_to_id") || (field == "updated_on"))
        options[:name] ||= l(options[:label] || "field_#{field}".gsub(/_id$/, ''))
      else
        @available_filters.delete(field)
      end
  ...
  def columns
    ...
    cols = (has_default_columns? ? default_columns_names : column_names).collect do |name|
      checkvar = name.inspect       
      if(checkvar == ":subject" || checkvar == ":assigned_to" || checkvar == ":priority" || checkvar == ":status")         
  ...
  def inline_columns_timelog
    columns_timelog.select(&:inline?)
  end

  def unapproved_timelog
    # preserve the column_names order
    cols = (has_default_columns? ? default_columns_names : column_names).collect do |name|
      checkvar = name.inspect
      if((checkvar != ":project") && (checkvar !=":issue"))
        available_columns.find { |col| col.name == name }
      end
    end.compact
    available_columns.select(&:frozen?) | cols
  ...
  def inline_unapproved_timelog
    unapproved_timelog.select(&:inline?)
  end
