...
class ApplicationController < ActionController::Base
  ...
  helper_method :check_usr_in_group
  helper_method :client_group_check
  ...
  before_filter :session_expiration, :user_setup, :check_if_login_required, :set_localization, :general_activity_id
  ...
  def general_activity_id    
    @general_activity_project = Project.where(:identifier=>"general-activity").first
  end
  ...
  def find_issues
    status = IssueStatus.find_by_id(params[:issue][:status_id]) if params[:issue].present? && params[:issue][:status_id].present?
    ...
    if status && params[:ids]
      if @client.empty?
        if status.name.capitalize == "Closed" && @reporter.empty?
  ...
  def check_usr_in_group
    #@group_manager_arr = Group.where(:lastname => "Managers").first
    #user_in_group_size = User.user_in_group(@group_manager_arr.id).size
    user_in_group_size = User.current.manager ? 1 : 0
      if user_in_group_size > 0
        return true
      else
        return false
      end
  ...
  def client_group_check
    @group_client_secured = Group.where(:lastname => "Clients").first
    user_in_client_secured = User.user_in_group(@group_client_secured.id).size
      if user_in_client_secured > 0
        return true
      else
        return false
      end
