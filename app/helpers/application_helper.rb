...
module ApplicationHelper
  ...
  def link_to_remove_fields(name, f)
    f.hidden_field(:_destroy) + link_to_function(name, "remove_fields(this)",class: "remove_icon")
  end
  ...
  def link_to_add_fields(name, f, association)
    ...
    link_to_function(name, "add_fields(this, \"#{association}\", \"#{escape_javascript(fields)}\")", class: "add_icon")
  ...
  def link_to_attachment_revision(attachment, options={})
    text = options.delete(:text) || attachment.filename
    route_method = options.delete(:download) ? :download_named_attachment_path : :named_attachment_path
    html_options = options.slice!(:only_path)
    url = send(route_method, attachment, attachment.filename, options)
  ...
  def render_flash_messages_sidebar
    s = ''
    flash.each do |k,v|
      s << content_tag('div', v.html_safe, :class => "flash #{k}", :id => "flash_#{k}")
    end
  ...
  def render_project_jump_box
    ...
    if projects.any?
      projects = projects.find_all{|project| project.parent_id!=nil}
      options =
  ...
  def principals_check_box_tags(name, principals)
    ...
    principals.each do |principal|
      s << "<label>#{ check_box_tag name, principal.id, false, :id => nil, :onclick => "checkClient(this, #{principal.id})" } #{h principal}</label>\n"
    end
    s.html_safe
  end
  ...
  def hours_for_timesheet
  [[0,0], [15,25], [30,50], [45,75]]
  end
  ...
  def minutes_append_zero(hours)
    mins = hours.to_s.split(".").second.to_i
    if mins==5
      mins = 50
    else
      mins
  ...
  def time_tag(time)
    ...
    if @project
      ...
      #link_to(text, "javascript:;", :title => format_time(time))
      text
    else
  ...
  def include_calendar_headers_tags
    # updated comment from redmine_vikram
    unless @calendar_headers_tags_included
      ...
      content_for :header_tags do
        ...
        if(Time.now.strftime("%w").to_i>3)
          min = Time.now.beginning_of_week.strftime("%Y-%m-%d")
        else
          min = (Time.now.beginning_of_week - 7.days).strftime("%Y-%m-%d")
        end
        ...
        #min = Time.now.strftime("%u") * -1

        tags = javascript_tag(
                   "var datepickerOptions={dateFormat: 'yy-mm-dd',  firstDay: #{start_of_week}, " +
                     "showOn: 'button', buttonImageOnly: true, minDate: '#{min}', maxDate: '#{max}', buttonImage: '" + 
  ...
  #To get sprint status
  def getSprintStatus(version_id)
    version = Version.find_by_id(version_id)
    version.present? ? (version.status == "closed" ? false : true) : false
  end
