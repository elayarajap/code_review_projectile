...
RedmineApp::Application.routes.draw do
  ...
  match 'projects/:id/sprints', :to => 'sprints#index', :via => :get, :as => 'project_sprint'

  match 'get_projects', :to => 'dashboard#get_projects', :as => 'get_projects', :via => [:get, :post]

  match 'management_api_report', :to => 'report_api#management_api_report', :as => 'management_api_report', :via => [:get]

  match 'fetch_all_groups', :to => 'report_api#fetch_all_groups', :as => 'fetch_all_groups', :via => [:get]

  match 'view_all_projects/:id', :to => 'dashboard#view_all_projects', :as => 'view_all_projects', :via => [:get, :post]

  match 'view_project_details/:id', :to => 'dashboard#view_project_details', :as => 'view_project_details', :via => [:get, :post]
 

  match 'update_approval_status', :to => 'timelog#update_approval_status', :as => 'update_approval_status', :via => [:get, :post]
  match 'attachments', :to => 'attachments#attachments', :as => 'attachments', :via => [:delete]
  #PM commented Starts
  ...
  #Pagination call for dashboard accounts
  match "get_accounts" => "dashboard#show", :as => :show_accounts, :via => [:get, :post]

  match "get_accounts_timesheet" => "dashboard#timesheet_account", :as => :timesheet_account, :via => [:get, :post]

  #TAG enhancement Starts
  match 'home_old', :to => 'welcome#index', :as => 'home_session_old', :via => [:get, :post]
  #TAG enhancement Ends

  #TAG enhancement Starts

  match 'home', :to => 'dashboard#projects', :as => 'home_session', :via => [:get, :post]
  

  match 'dashboard', :to => 'dashboard#projects', :as => 'dashboard', :via => [:get, :post]
  #TAG enhancement Ends
  ...
  match 'users/check_role', :to => 'users#check_role', :via => :get
  resources :users
  ...
  resources :projects do
    member do
      ...
      get 'meeting'
      get 'new_meeting'
      match 'create_meeting', :via => [:get, :post]
      match 'update_meeting', :via => [:put]
      match 'add_to_favorite', :via => [:get, :post]
      match 'remove_favorite', :via => [:get, :post]
    end
    ...
    match 'issues/create_by_csv', :controller => 'issues', :action => 'create_by_csv', :via =>[:post]

    resources :files, :only => [:index, :new, :create]

    
    resources :directories, :only => [:index, :new, :edit, :update, :create, :destroy]

    resources :meetings

    
    #match '/directories/:id', :to => 'directories#destroy', :via => :get, :as => 'delete_directories'

    match 'projects/:id/directories/:id', :to => 'directories#destroy', :via => :delete
  ...
  resources :documents, :only => [:show, :edit, :update, :destroy] do
    ...
    get 'send_for_approval', :on => :member
    get "approve", :on => :member
    get "reject", :on => :member
    get "client", :on => :member
    get 'create_conversation', :on => :member
    get 'delete_conversation', :on => :member
  end
