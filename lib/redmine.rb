...
Redmine::AccessControl.map do |map|
  map.permission :view_project, {:projects => [:show, :initiation], :activities => [:index]}, :public => true, :read => true
...
Redmine::MenuManager.map :top_menu do |menu|
  ...
  menu.push :home, { :controller => 'welcome', :action => 'index' }, :if => Proc.new { User.current.logged? }
  ...
  #PM commented Ends
  #TAG enhancement Starts
  menu.push :help, Redmine::Info.help_url, :last => true, :if => Proc.new { User.current.logged? }
  #TAG enhancement Ends 
end

Redmine::MenuManager.map :account_menu do |menu|
  menu.push :login, :signin_path, :html => {:title => "Login"}, :if => Proc.new { !User.current.logged? }
  menu.push :register, :register_path, :html => {:title => "Register"}, :if => Proc.new { !User.current.logged? && Setting.self_registration? }
  menu.push :settings, {:controller => 'settings'}, :html => {:title => "Settings"}, :if => Proc.new { User.current.admin? }
  menu.push :time_entries, {:controller => 'timelog'}, :html => {:title => "Time Entries"}, :if => Proc.new { User.current.logged? }
  menu.push :my_account, { :controller => 'my', :action => 'page', :my_task=>'true' }, :html => {:title => "My Account"}, :if => Proc.new { User.current.logged? }
  menu.push :logout, :signout_path, :html => {:method => 'post', :title => 'Logout'}, :if => Proc.new { User.current.logged? }
end
...
Redmine::MenuManager.map :project_menu do |menu|
  menu.push :overview, { :controller => 'projects', :action => 'show' }, :caption => 'Sprint View'
  menu.push :todo, { :controller => 'issues', :action => 'index', :tid => "2" }, :param => :project_id, :caption => 'ToDo'
  # menu.push :addtodo, { :controller => 'issues', :action => 'new', :tid => "2", :copy_from => nil }, :param => :project_id, :caption => 'Add ToDo',
  #              :html => { :accesskey => Redmine::AccessKeys.key_for(:addtodo) }
  menu.push :bugs, { :controller => 'issues', :action => 'index', :tid => "1" }, :param => :project_id, :caption => 'Bugs'
  menu.push :details, { :controller => 'timelog', :action => 'index', :sub_project =>true }, :params => :project_id, :caption => 'Timesheet'
  # menu.push :addbugs, { :controller => 'issues', :action => 'new', :tid => "3", :copy_from => nil }, :param => :project_id, :caption => 'Add Bugs',
  #              :html => { :accesskey => Redmine::AccessKeys.key_for(:addtodo) }             
  ...
  #menu.push :initiation, { :controller => 'projects', :action => 'initiation' }, :caption => 'Initiation'
