  ...
  module Acts
    module Attachable
      ...
      module InstanceMethods
        ...
        def save_attachments_with_revision(attachments, obj1, author=User.current)
          if attachments.is_a?(Hash)
            attachments = attachments.stringify_keys
            attachments = attachments.to_a.sort {|a, b|
              if a.first.to_i > 0 && b.first.to_i > 0
          ...
          if attachments.is_a?(Array)
            attachments.each do |attachment|
              next unless attachment.is_a?(Hash)
              a = nil
              if file_with_revision = attachment['file']
                revision_file = attachment['file']
                revision_file.original_filename = "#{obj1}_"+revision_file.original_filename
                next unless file_with_revision.size > 0
                a = Attachment.create(:file_with_revision => attachment['file'], :author => author, :revision => obj1)
              elsif token = attachment['token']
                a = Attachment.find_by_token(token)
                next unless a
                a.filename = "#{obj1}_"+attachment['filename'] unless attachment['filename'].blank?
                a.content_type = attachment['content_type']
                a.revision = obj1
              end
              next unless a
        ...
        def save_attachments_with_directory(attachments, obj1, author=User.current)
          if attachments.is_a?(Hash)
            attachments = attachments.stringify_keys
            attachments = attachments.to_a.sort {|a, b|
              if a.first.to_i > 0 && b.first.to_i > 0
          ...
          if attachments.is_a?(Array)
            attachments.each do |attachment|
              ...
              if file = attachment['file']
                next unless file.size > 0
                a = Attachment.create(:file => file, :author => author, :directory_id => obj1)
              elsif token = attachment['token']
                a = Attachment.find_by_token(token)
                next unless a
                a.filename = attachment['filename'] unless attachment['filename'].blank?
