...
module Redmine
  module Helpers
    class TimeReport
      attr_reader :criteria, :columns, :hours, :total_hours, :periods, :non_billable

      def initialize(project, issue, criteria, columns, time_entry_scope, hours_type)
        @project = project
        @issue = issue
      ...
      def run
        unless @criteria.empty?
          ...
          @hours = []          
          #Modified by himanth for showing approved billable time for all reports
          if @hours_type!='4'
          ...
          if !@hours_type.empty?
            if @hours_type=='1'
                ...
                #Rails.logger.debug "hours values #{hours}"
                #mod_hours = hours.to_f.round(2)
                #Rails.logger.debug "mod hours values #{mod_hours}"
                h = {'hours' => hours} 
                ...
                Rails.logger.debug "total hours values #{@hours}"
              end
            elsif @hours_type=='2'
                  @scope.sum(:billable,
                  :include => [:issue, :activity],
          ...
          #@total_hours = TimeEntry.sum_hours(@scope.map{|hash| hash['hours'].to_f.round(2)})

          #Rails.logger.debug "today hours values #{@hours}"
